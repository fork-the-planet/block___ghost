...
The `none` → `deliberate-none` change is breaking for any `fingerprint.md` setting `shadowComplexity: none`. `ghost-scan` is pre-1.0 and not yet published, so no major bump; existing files should update their value.
