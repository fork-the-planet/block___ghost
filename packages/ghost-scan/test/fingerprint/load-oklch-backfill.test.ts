...
describe("loadFingerprint — backfills palette oklch", () => {
  let dir: string;
  ...
  it("populates oklch on dominant colors when missing from frontmatter", async () => {
    ...
    await writeFile(path, HEX_ONLY_FINGERPRINT);

    const { fingerprint } = await loadFingerprint(path, {
      noEmbeddingBackfill: true,
    });

    const dominant = fingerprint.palette.dominant[0];
  ...
  it("populates oklch on semantic colors when missing", async () => {
    ...
    const danger = fingerprint.palette.semantic.find(
      (c) => c.role === "danger",
    );
  ...
  it("is deterministic — two loads of the same file produce identical oklch", async () => {
    ...
    const a = await loadFingerprint(path, { noEmbeddingBackfill: true });
    const b = await loadFingerprint(path, { noEmbeddingBackfill: true });

    expect(a.fingerprint.palette.dominant[0].oklch).toEqual(
      b.fingerprint.palette.dominant[0].oklch,
    );
  });

  it("preserves existing oklch when present (does not recompute)", async () => {
    const withExisting = HEX_ONLY_FINGERPRINT.replace(
      `- { role: ink, value: "#1a1a1a" }`,
    ...
    // when missing.
    expect(fingerprint.palette.dominant[0].oklch).toEqual([0.5, 0.1, 200]);
  });
});
