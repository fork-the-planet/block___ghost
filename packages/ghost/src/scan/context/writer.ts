...
export function buildSkillMd(
  ...
  const fileList = [
    "- `fingerprint.md` — deprecated legacy direct-markdown design-language context (YAML digest + Character/Signature/References/Decisions)",
    "- `prompt.md` — generation prompt distilled from legacy fingerprint.md",
    ...(includesCss
      ? [
...
5. **Checks** — embedded legacy gate hints when available

When generating UI in this language:

- Treat **Checks** as curated gates when they are provided.
...
function buildPromptMd(fingerprint: Fingerprint, name: string): string {
  const parts: string[] = [];
  parts.push(
    `You are generating UI in the **${name}** design language from a legacy direct-markdown Ghost context. Produce the requested UI artifact; explain design decisions only when the user asks.`,
  );
...
function buildReadmeMd(fingerprint: Fingerprint, name: string): string {
  ...
  const personalityLine = personality.length
    ...
    : "";
  return `# ${name} — legacy design context bundle

Generated from deprecated direct-markdown input. Grounding material for AI UI generation in the **${name}** design language.${personalityLine}

## Files

- \`SKILL.md\` — Agent Skill manifest (user-invocable)
- \`fingerprint.md\` — legacy direct-markdown design-language context (YAML frontmatter + Character/Signature/Decisions)
...
function buildSkillDescription(fingerprint: Fingerprint, name: string): string {
  ...
  const traitPhrase = personality.length
    ...
    : "";
  return `Use this legacy skill to generate UI in the ${name} design language${traitPhrase}. Contains direct-markdown fingerprint and token reference.`;
}
