...
export async function resolveParent(
  ...
  switch (target.type) {
    ...
    default:
      throw new Error(
        `Cannot resolve parent fingerprint from target type "${target.type}". Generate one first by running the profile recipe in your host agent (install with "ghost-drift emit skill").`,
      );
  }
}
